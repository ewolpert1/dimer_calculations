...
def main() -> None:
    ...
    facet_axes, centroid_to_facet = Axes.ByMidpoint(
        molecule,
    ...
    list_of_edges, centroid_to_edge = Axes.BySmiles(
    ...
    For this you need:
        ...
        closer)
        displacement: float. This+displacement_distance is the maximum
    ...
    It returns a dictionary with the following information:
        ...
        'Displacement centroid': The actual displacement of the centroid of
        the neighbouring cage
    ...
    For this you need:
        ...
        metal_atom: int. The atom index of the metal atom. If none then it
        will just fix the smiles string.
    ...
    fixed_atom_set = CageOperations.fix_atom_set(
        ...
        utils.remove_aldehyde("NCCN"),
        metal_atom=None,
    )

    """
    This is the optimisation part. It will optimise the dimer using GULP,
    OPLS or XTB.
    ...
    This is what you'll need to add your own CGOptimiser or something to. You
    can do the in the DimerOptimizer class in the Optimiser_functions.py file.
    ...
    for dimer_entry in list_of_dimers:
        if not os.path.exists("dimers"):
            os.makedirs("dimers")
        filename = (
            f"dimers/XTB_shell_{dimer_entry['Displacement shell']}_slide_"
            f"{dimer_entry['Slide']}_rot_{dimer_entry['Rotation']}.mol"
        )
        dimer_entry["Dimer"].write(filename)
        rdkit_molecule = Chem.MolFromMolFile(filename)
        # This threshold is the minimum distance allowed between non hydrogen
        # atoms
        overlap = dimer_calculations.check_overlaps(
            rdkit_molecule, threshold=1
        )
        dimer_calculations.optimise_dimer_XTB(
            dimer_entry["Dimer"],
            (
