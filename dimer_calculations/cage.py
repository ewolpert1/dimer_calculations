...
class CageOperations:
    ...
    def __init__(self, cage: stk.Molecule) -> None:
        """Initialize CageOperations."""
        self.cage = cage
    ...
    def generate_new_cages(
        ...
        """
        perpendicular_vector_w = calculate_perpendicular_vector(
            direction_vector_w
        )
        perpendicular_vector_a = calculate_perpendicular_vector(
            direction_vector_a
        )
        list_wa, list_ww, list_aa, list_wv = [], [], [], []
        for i in np.arange(0, 7, 1):
            displaced_centers_w = find_integer_points(
                direction_vector_w,
                displacement_distance * 2 - 2 + i,
                int(displacement_distance) + 1,
            )
            displaced_centers_a = find_integer_points(
                direction_vector_a,
                displacement_distance * 2 - 2 + i,
                int(displacement_distance) + 1,
            )
                ...
                for rotation_vector in range(len(rotated_vectors_w)):
                    ...
                    complex_wa = stk.ConstructedMolecule(
                        topology_graph=stk.host_guest.Complex(
                            host=self.cage, guests=guest_wa
                        )
                    )
                    complex_ww = stk.ConstructedMolecule(
                        topology_graph=stk.host_guest.Complex(
                            host=self.cage, guests=guest_ww
                        )
                    )
                    complex_aa = stk.ConstructedMolecule(
                        topology_graph=stk.host_guest.Complex(
                            host=self.cage, guests=guest_aa
                        )
                    )
                    complex_wv = stk.ConstructedMolecule(
                        topology_graph=stk.host_guest.Complex(
                            host=self.cage, guests=guest_wv
                        )
                    )
    ...
    def cage_vertex_vec(self, amine_smile):
        """Get the vector of average cage-arene distance."""
        rdkit_mol = self.cage.to_rdkit_mol()
        rdkit.SanitizeMol(rdkit_mol)
        arenes = []
        for atom_ids in rdkit_mol.GetSubstructMatches(
            ...
            centroid = self.cage.get_centroid(atom_ids=atom_ids)
    ...
    def find_midpoint_of_facet(self, vectors, sym, tolerance=0.1):
        ...
        all_distances = [
            distance(v1, v2) for v1, v2 in itertools.combinations(vectors, 2)
        ]
    ...
    def displace_cages(self, arene_smile, diamine_smile, sym, metal_atom):
        dist = self.cage_size(arene_smile)
        # this has all the vectors of the arene to centroid
        vecs_vertex = self.cage_vertex_vec(diamine_smile)
        vec_vertex = vecs_vertex[0]
        if sym == "4_6":
            vec_w = -self.calculate_cage_vector(arene_smile)
            vec_a = self.calculate_cage_vector(arene_smile)
        elif sym in ("oct", "trunc_oct"):
            vec_w = self.find_midpoint_of_facet(vecs_vertex, sym)
            vec_a = self.calculate_cage_vector(arene_smile)

        if sym == "4_6":
            guest_bb_wa = stk.BuildingBlock.init_from_molecule(self.cage)
            guest_bb_aa = stk.BuildingBlock.init_from_molecule(self.cage)
            ...
            guest_bb_ww = stk.BuildingBlock.init_from_molecule(self.cage)
        ...
        origin = guest_bb_wa.get_centroid()
        rotated_vectors_w = generate_rotated_vectors(vec_w, 4, 30)
        rotated_vectors_a = generate_rotated_vectors(vec_a, 4, 30)

        list_wa, list_ww, list_aa, list_wv = self.generate_new_cages(
            guest_bb_aa,
            guest_bb_ww,
            guest_bb_wa,
            guest_bb_wv,
        ...
        # list_wv = CageOperations.generate_ww_cages(guest_bb_wa, guest_bb_
        # wv, vec, vec_perpendicular, rotated_vectors, dist)
