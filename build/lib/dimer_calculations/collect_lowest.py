...
def mol_from_mae_file(Cagename,destination_folder_end,mae_end):
    ...
    for atom_block, bond_block, structure_name in zip(atom_blocks, bond_blocks, structure_names):
        mol = rdkit.EditableMol(rdkit.Mol())
        conf = rdkit.Conformer()
...
def log_to_mol(Cagename,destination_folder_end,log_file="no_constraints.log"):
    ...
    with open(f'{destination_folder}/{log_file}', 'r') as f:
        file_content = f.read()
        structure_name_matches = re.findall(r'Structure name, if any, appears on next line:\s*(.*?)\s', file_content)
    ...
    with open(output_filename, "w+") as input1, open(f'{destination_folder}/{log_file}', 'r') as f:
        ...
        for line in lines:
            if 'Total Energy' in line:
                ...
                energy_counter = 0  # Reset energy counter for the next set of energy values
...
#
        if minima_for_file:
            ...
            filtered_minima = [(energy, name) for energy, name in sorted(minima_for_file) if energy <= min_energy + 50]
