...
"""
This function returns the vectors between the centroid of the molecule and the
smiles string and the magnitude/distance.

You need to give it:
    molecule: stk.Molecule
    smiles_string: str. Here there is a remove_aldehyde function you can use
    which takes away the aldehydes so you can use the precursor string exactly
...
"""
You can then take these vectors and use them to determine the centroids of the
facets, even if theyre windows.

For this you need to give it:
    molecule: stk.Molecule
    vectors: list of vertices
    vertice_size: float
    no_vectors_define_facet: int. This is the number of vectors that define a
    facet. For example for a 6+4 cage it is 3 but for truncated octahedron it
    could be 6
...
For this you need:
    ...
    funciton rotates the molecule so the vector you give is facing the original
     cage. For window to arene you just need it the same, but for
     window-to-window or window-to-arene the cage must be inverted, hence why
     it is -axis.
    ...
    closer)
    displacement: float. This+displacement_distance is the maximum distance of
    displacement
...
It returns a dictionary with the following information:
    ...
    'Slide': Which position along the face you're at if you're sliding, doesnt
     actually equate to where but 0 is on axis and not 0 is off axis
...
If you want to constrain the atom positions this function will return the atom
positions of the cage you want to fix. It constrains the smiles string unless
metal_atom is not none.
...
I'm pretty sure this only works if your molecule string contains an imine. I
imagine you wont use this so wont edit it to be better but lmk if you do want
it.

"""

fixed_atom_set = CageOperations(cage=list_of_dimers[0]["Dimer"]).fix_atom_set(
    remove_aldehyde("NCCN"),
    metal_atom=None,
)

"""
This is the optimisation part. It will optimise the dimer using GULP, OPLS or
XTB.
...
For this you need:
    ...
    XTB_PATH
    fixed_atom_set: set. The set of atoms you want to fix, assumed none if not
     there.
