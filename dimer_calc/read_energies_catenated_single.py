...
class OPLSEnergyReader:
    def __init__(self, cage_name_mae, cage_name, current_directory, destination_folder_end):
            ...
            if folder_name.startswith(self.cage_name_mae):
                ...
                for filename in filenames:
                    if filename.endswith(".log"):  # Identify log files
                        self.process_log_file(dirpath, filename, all_minima, merge_file_path)
        self.finalize_merge_file(merge_file_path)
        sorted_minima = sorted(all_minima, key=lambda x: x[0])
        energies_file_path = os.path.join(self.destination_folder, "Constrained_energies_OPLS.txt")
        ...
        for energy, structure_name in sorted_minima:
            ...
            if not catenated:
                ...
                if reference_energy is not None:
                    self.update_merge_file(merge_file_path, structure_name, dirpath)
    ...
    def run_calc(self):
        ...
        subprocess.run("sh merge.txt", shell=True)
        os.environ["SCHRODINGER"] = SCHRODINGER_PATH
        subprocess.run(f"{SCHRODINGER_PATH}/bmin -WAIT no_constraints", shell=True)
        os.chdir(self.current_directory)

class GULPEnergyReader:
    def __init__(self, cage_name_gulp, cage_name, current_directory, destination_folder_end):
    ...
    def read_energy(self, output_file='Constrained_energies_GULP.txt'):
        ...
        all_minima=[]
        for file_path in glob.glob(f'{search_dir}/**/gulp_opt.ginout', recursive=True):
...
class XTBEnergyReader:
    def __init__(self, cage_name_xtb, cage_name, current_directory, destination_folder_end):
        ...
        with open(filename, 'r') as file:
            ...
            for line in file:
                ...
                if "TOTAL ENERGY" in line:
                    energy = float(line.split()[3])  # Assuming the energy value is the fourth element
                    #print(filename[:-4])
