...
def main() -> None:
    ...
    For this you need to give it:
        ...
        vertice_size: float
        no_vectors_define_facet: int. This is the number of vectors that
        define a facet. For example for a 6+4 cage it is 3 but for truncated
    ...
    list_of_arenes, centroid_to_arene = Axes.BySmiles(
    ...
    atom positions of the cage you want to fix. It constrains the smiles string
    unless metal_atom is not none.
    ...
    for dimer_entry in list_of_dimers:
        optimise_dimer_OPLS(
            dimer_entry["Dimer"],
            f"OPLS_shell_{dimer_entry['Displacement shell']}_slide_{dimer_entry['Slide']}_rot_{dimer_entry['Rotation']}",
            SCHRODINGER_PATH,
            fixed_atom_set,
        )
